    ...
    <View className="flex-1 p-4">
      <View className="flex-1 rounded-lg bg-white">
        {!showRating ? (
          <View>
            <View
              style={{ boxShadow: '2px 2px 4px 0px rgba(82, 165, 135, 0.15)' }}
            ...
            <View className="mt-6">
              <View
                style={{ boxShadow: '2px 2px 4px 0px rgba(82, 165, 135, 0.15)' }}
                className="pt-3rounded-md px-3 pb-2">
          ...
          <View
            style={{ boxShadow: '2px 2px 4px 0px rgba(82, 165, 135, 0.15)' }}
            className="min-h-[45vh] flex-1 rounded-md p-4">
            <ScrollView>
        ...
        <View className="relative rounded-2xl">
          <ScrollView
            ...
            {images.map((image, index) => (
              <View key={index} className="h-64 w-screen overflow-hidden rounded-lg">
                <Image
                  source={image}
                  className="mx-auto h-full w-[94%] rounded-lg"
                  resizeMode="cover"
                />
              </View>
            ))}
          ...
          {/* Navigation Arrows */}
          <TouchableOpacity
            className="absolute left-6 top-1/2 -translate-y-1/2 rounded-full bg-[#FFFFFFCC] p-2"
            onPress={() => handleSlideChange('prev')}>
            <Ionicons name="chevron-back" size={24} color="#1C1B1F" />
          </TouchableOpacity>
          <TouchableOpacity
            className="absolute right-6 top-1/2 -translate-y-1/2 rounded-full bg-[#FFFFFFCC] p-2"
            onPress={() => handleSlideChange('next')}>
            <Ionicons name="chevron-forward" size={24} color="#1C1B1F" />
          </TouchableOpacity>
        </View>

        {/* School Info */}
        <View className="p-4">
          <Text className="text-xs font-medium">Rowad al khaleej international school</Text>
          <View className="mt-5 flex-row items-center">
            <Ionicons name="location" size={16} color="#666" />
            <Text className="ml-1 text-[10px] text-[#868686]">UAE, Dubai</Text>
            <Text className="ml-4 text-[10px] text-[#868686]">10 seats remaining</Text>
          </View>
        </View>
        ...
        <View className="flex-row border-b border-gray-200 px-4">
          {['information', 'courses offered', 'review'].map((tab) => (
            <TouchableOpacity
              ...
              <Text
                className={`${activeTab === tab ? 'font-semibold text-[#52A587]' : 'text-[#868686]'} text-sm capitalize`}>
                {tab}
              </Text>
            </TouchableOpacity>
          ))}
          ...
          <View className="flex-row items-center justify-between border-t border-gray-200 bg-white px-4 py-3">
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                ...
                const numberWithCode = formattedNumber.startsWith('+')
                  ? formattedNumber
                  : `+${formattedNumber}`;

                const whatsappUrl = `whatsapp://send?phone=${numberWithCode}`;
                const webWhatsappUrl = `https://wa.me/${numberWithCode}`;
